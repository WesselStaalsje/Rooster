...
export type TemplateField = {
  ...
  group: string;
  label: string;
  addr: string;           // target cell to write
  sheetName: string;
  sortKey: string;
  styleFromAddr?: string; // copy style if target cell is missing/un-styled
};
...
function findCellByText(ws: any, text: string): FoundCell | null {
  const target = norm(text);
  for (const k of Object.keys(ws)) {
    if (!isCellAddress(k)) continue;
    const cell = ws[k];
    const v = norm(safeString(cell?.v));
    if (v === target) {
...
function extractTwoColBlock(ws: any, sheetName: string, headerText: string, maxRows = 40): TemplateField[] {
  const header = findCellByText(ws, headerText);
  if (!header) return [];

  const fields: TemplateField[] = [];
  const timeCol = header.c;
  const nameCol = header.c + 1;
...
function extractKeyValue(ws: any, sheetName: string, labelText: string, group: string): TemplateField[] {
  const cell = findCellByText(ws, labelText);
  if (!cell) return [];
...
function extractZwareBergingRows(ws: any, sheetName: string, group: string, rowLabels: string[]): TemplateField[] {
  ...
  for (const label of rowLabels) {
    const areaCell = findCellByText(ws, label);
...
export async function loadTemplateWorkbook(): Promise<any> {
  ...
  // Cruciaal: styles + number formats inlezen, anders raak je opmaak kwijt.
...
export function setCellString(ws: any, addr: string, value: string, styleFromAddr?: string) {
  const existing = ws[addr];

  if (!existing) {
    ws[addr] = { t: "s", v: value ?? "" };
    // probeer style over te nemen zodat opmaak behouden blijft in "lege" cellen
    if (styleFromAddr && ws[styleFromAddr]) {
      if (ws[styleFromAddr].s) ws[addr].s = structuredClone(ws[styleFromAddr].s);
      if (ws[styleFromAddr].z) ws[addr].z = ws[styleFromAddr].z;
    }
    return;
  }

  // behoud style van bestaande cel
  existing.t = "s";
  existing.v = value ?? "";
  ws[addr] = existing;
}
